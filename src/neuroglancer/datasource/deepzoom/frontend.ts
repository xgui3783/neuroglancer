...
(WithParameters(VolumeChunkSource, ImageTileSourceParameters)) {}
...
/*export*/ class DeepzoomPyramidalImageTileSource extends MultiscaleVolumeChunkSource {
  ...
  constructor(
      chunkManager: ChunkManager, /*public*/ url: string, public info: PyramidalImageInfo) {
    super(chunkManager);
...
function getDZIMetadata(
    chunkManager: ChunkManager, url: string): Promise<DZIMetaData> {
...
async function getImageDataSource(
    options: GetDataSourceOptions, url: string, metadata: DZIMetaData): Promise<DataSource> {
  const info = buildPyramidalImageInfo(metadata);
  const volume =
      new DeepzoomPyramidalImageTileSource(options.chunkManager, url, info);
  ...
  get(options: GetDataSourceOptions): Promise<DataSource> {
    ...
    return options.chunkManager.memoize.getUncounted(
        {'type': 'deepzoom:get', providerUrl, parameters}, async(): Promise<DataSource> => {
          ...
          const metadata = await getDZIMetadata(options.chunkManager, url);
          return getImageDataSource(options, url, metadata);
        });
  }
